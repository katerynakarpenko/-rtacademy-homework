...
const createPostElement = ( post ) =>
{
    ...
    el_article.setAttribute ('class', 'post');

    // створюємо елемент a.cover
    const el_a_cover       = document.createElement( 'a' );
    el_a_cover.setAttribute( 'class', 'travel' );
    ...
    el_a_description.setAttribute( 'class', 'text description' );
    ...
    el_a_more.setAttribute( 'class', 'read-more-post' );
...
const loadMorePosts = async ( event ) =>
{
    ...
    let
        page        = parseInt( document.getElementById( 'downloadMore' ).getAttribute( 'data-current-page' ) ) || 1,
        maxPages    = parseInt( document.getElementById( 'downloadMore' ).getAttribute( 'data-max-pages' ) ) || 1;

    // додаткова перевірка одразу
    if( page > maxPages )
    {
        // видаляємо кнопку "Load More", бо ми знаємо що більше постів не буде
        document.getElementById('downloadMore').remove();
        return false;
    }
    ...
    if( response.ok )           // HTTP = 200, все ОК
    {
        ...
        document.getElementsByClassName('all-posts')[0].append( ...postsElements );
        // "..." - це оператор spread
        ...
        else
        {
            ...
            document.getElementById( 'downloadMore' ).setAttribute( 'data-current-page', page );
        }
    }
    else
...
if( document.getElementById( 'downloadMore' ) )
{
    document.getElementById( 'downloadMore' ).onclick = loadMorePosts;
}